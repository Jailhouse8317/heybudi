## User Story 3: The Non-Programmer Logic Assistant (GitHub Copilot Lifecycle)

**Persona:** Alex, Year 3 student at the School of Engineering (SEG).

**The Story:** > As an engineering student who needs to write complex hardware-programming logic for my technical project,
...
- It must provide real-time, greyed-out inline code predictions (e.g., Arduino C++ or Python loops) inside Alex's IDE (like VS Code) based on the hardware components he is trying to program.
- It must feature an inline chat panel that allows him to highlight a broken block of engineering logic, get an immediate plain-English explanation of the bug, and receive a secure refactor fix.
- It must be fully activated using the free GitHub Education Student Admin Card verification tier, ensuring his academic project data remains securely sandboxed.
...
## Prerequisites & Verification (Zero-Cost & Secure)

1. Open **VS Code** and ensure the **GitHub Copilot** and **GitHub Copilot Chat** extensions are installed.
2. Sign in to your GitHub account. Ensure your account is verified under the **GitHub Education Student Developer Pack** (your free academic tier).
...
1. In the Copilot Edits prompt bar, dictate a high-level architectural instruction instead of writing raw code blocks:
    ...
    > 
2. Press Enter.
3. **The Agent Action:** Copilot will not just give a text response. It acts as an agent—it scans your workspace dependencies, plans a multi-file structural edit, and updates the targeted files simultaneously.

### Step 3: Reviewing and Testing the Agent's Diffs

1. As Copilot edits your code, you will see a live summary of modified files and an inline code diff layout within your editor window.
2. Click into a modified file to view the changes side by side (Green highlights represent optimization fixes; Red highlights represent removed bottlenecks).
3. Open your integrated VS Code terminal and compile or run your hardware environment script.
4. **If a Compilation Error Occurs:** You don't need to debug it manually. Simply type in the Edits panel:
...
3. If an approach doesn't work out as intended, click **"Undo/Reset"** to instantly revert all modified files back to your original baseline state without losing any data.